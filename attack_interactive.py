...
def visualize(input_str, dict_word=[], index2word=[], classes_list=[], power=5, scoring_alg='replaceone',
              transformer_alg='homoglyph', model=models, mode='word', maxlength=500, device=None,
              filter_char=default_filter, alphabet=default_alphabet):
    ...
    if mode == 'word':
        ...
        while j < power and t < input_seq.size()[1]:
            if advinputs[0, indices[0][t]].item() > 3:
                word, advinputs[0, indices[0][t]] = transformer.transform(transformer_alg)(
                    advinputs[0, indices[0][t]].item(), dict_word, index2word, top_words=20000)
        ...
        return (input_str, torch.exp(res1).detach().cpu()[0], classes_list[pred1], adv_str,
        ...
        while j < power and t < inputs.size()[2]:
            if dt[0, indices[0][t]].item() > 0:
                advinputs[0, :, indices[0][t]], nowchar = transformer_char.transform(transformer_alg)(inputs, torch.max(
                    advinputs[0, :, indices[0][t]], 0)[1].item(), alphabet)
        ...
        return (input_str, torch.exp(output)[0].detach().cpu(), classes_list[pred1.item()], md,
                torch.exp(output2)[0].detach().cpu(), classes_list[pred2.item()])
    else:
...
if __name__ == '__main__':
    ...
    numclass = len(classes_list)
    if args.model == "charcnn":
        model = models.CharCNN(classes=numclass)
    elif args.model == "simplernn":
        model = models.SmallRNN(classes=numclass)
    elif args.model == "bilstm":
        model = models.SmallRNN(classes=numclass, bidirection=True)
    ...
    while s:
        visualize(s, power=2, mode=args.datatype, model=model, dict_word=word_index, index2word=index2word,
                  classes_list=classes_list, device=device)
