...
This module provides the classes that are used to load outputs used
by botocore.  This can include:

    * Service outputs (e.g. the model for EC2, S3, DynamoDB, etc.)
    * Service model extras which customize the service outputs
    * Other outputs associated with a service (pagination, waiters)
...
will attempt to load outputs outside of the outputs we ship with
...
``~/.aws/outputs`` without having to mess around with the AWS_DATA_PATH.
...
this structure for service outputs::
...
information that doesn't quite fit in the original outputs, but is still needed
...
class JSONFileLoader(object):
    ...
    This class can load the default format of outputs, which is a JSON file.
...
class Loader(object):
    """Find and load data outputs.

    This class will handle searching for and loading data outputs.
    ...
    # The included outputs in botocore/data/ that we ship with botocore.
    BUILTIN_DATA_PATH = os.path.join(BOTOCORE_ROOT, 'data')
    # For convenience we automatically add ~/.aws/outputs to the data path.
    ...
    def load_service_model(self, service_name, type_name, api_version=None):
        ...
        This is the main method for loading botocore outputs (e.g. a service
...
class ExtrasProcessor(object):
    """Processes data from extras files into service outputs."""
    def process(self, original_model, extra_models):
        ...
        :param extra_models: A list of loaded extras outputs.
        """
